...
# 同一子流程内对“搜索”类工具的调用次数上限。模型的提示词鼓励“坚持不懈 /
# 如果第一次搜索没有结果，请扩大查询范围”，当数据库里没有匹配结果时可能反复调用
# 搜索工具形成长循环，导致步数爆炸、超过流式预算而超时（表现为“一直思考、刷新后无答案”）。
# 超过上限后直接退出子流程，交由主助手给出结论，避免卡死。
_MAX_HOTEL_SEARCH_ATTEMPTS = 2
_MAX_FLIGHT_SEARCH_ATTEMPTS = 3


def _count_tool_calls(messages, tool_name: str) -> int:
    """统计历史消息中某工具被调用的次数（用于限制搜索类工具的重复调用）。"""
    n = 0
    for m in messages:
...
def build_flight_graph(builder: StateGraph) -> StateGraph:
    ...
    def route_update_flight(state: dict):
        ...
        if all(tc["name"] in safe_tool_names for tc in tool_calls):  # 如果所有调用的工具都是安全工具
            # 防止反复搜索同一航班形成长循环：超过上限直接退出子流程
            if _count_tool_calls(state["messages"], "search_flights") > _MAX_FLIGHT_SEARCH_ATTEMPTS:
                return "leave_skill"
...
def builder_hotel_graph(builder: StateGraph) -> StateGraph:
    ...
    def route_book_hotel(state: dict):
        ...
        if all(tc["name"] in safe_toolnames for tc in tool_calls):  # 如果所有调用的工具都是安全工具
            # 防止反复搜索同一酒店形成长循环：超过上限直接退出子流程
            # 酒店子助手现在使用高德 POI 搜索（amap_search_poi / amap_search_around）
            amap_search_count = (
                _count_tool_calls(state["messages"], "amap_search_poi")
                + _count_tool_calls(state["messages"], "amap_search_around")
            )
            if amap_search_count > _MAX_HOTEL_SEARCH_ATTEMPTS:
                return "leave_skill"
